...
C<unsafe=0> will be the default.  C<buildSchedule> will warn about deprecation.

=item 0.2.0

C<buildSchedule> will be removed; only the object interface will be supported.
...
Planned improvements include helpers to merge annotation attributes into schedules, additional reporting functions, and automatic mapping of attributes for some action node parameters (eg limits).  As of 0.1.9, attributes may be "fixed" at activity boundaries but that is no longer done automatically.

=head2 Annotations

Planned improvements include helpers to merge annotation attributes into schedules.  This also requires options to control the order/materialization of annotation generation.  Proposed is annotation configuration per-node (with a keyname, possibly duplicated in the upper-level configuration).
