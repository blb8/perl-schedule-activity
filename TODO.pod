...
=item Automatic attributes per node (done)

=item Remove historically completed items from the list below (todo)

For summarization, features below often specify availability "starting with" a version.  Documentation has been reorganized and rewritten.  Expect feature lists below to remove any completed, non-experimental items.
...
Automatic per-node attributes are available with 0.2.9.  They support a configurable prefix, a script-based reporting mechanism, node filtering, and goal seeking (by y-value).
