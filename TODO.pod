...
=item Documentation rewrite

=item Goal scheduling
...
Scheduling goals are planned of the form C<cycles=N>, C<attribute=E<gt>{name=E<gt>{op=E<gt>'max|min'},name=E<gt>{op=E<gt>'eq|ne',value=E<gt>x}}>.  Rescheduling will repeat through C<cycles> and the schedule with the highest score will be returned.  Only attribute-based goals seeking is planned at this time.
